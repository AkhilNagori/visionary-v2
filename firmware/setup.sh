...
# Run from the repo:  sudo bash firmware/setup.sh [--with-whisper]
...
for arg in "$@"; do
  case "$arg" in
    ...
    *) echo "unknown option: $arg (supported: --with-whisper)" >&2; exit 2 ;;
  esac
done
...
apt-get install -y python3-picamera2 python3-gpiozero python3-requests \
  ...
  espeak-ng alsa-utils sox avahi-daemon rsync

echo "== 2/8 I2S audio overlay (MAX98357A out + ICS-43434 in) =="
...
# Each file goes to its own name (the old script clobbered the .onnx with the .json).
...
sox -n -r 22050 -c 1 "$S/rec_start.wav" synth 0.20 sine 440:1040 vol 0.5
sox -n -r 22050 -c 1 "$S/rec_stop.wav"  synth 0.20 sine 1040:440 vol 0.5

echo "== 6/8 install app -> $APP =="
mkdir -p "$APP"
...
if [ "$WITH_WHISPER" -eq 1 ]; then
  echo "== 8/8 whisper.cpp (offline STT) =="
  ...
  echo "== 8/8 whisper.cpp skipped (re-run with --with-whisper for offline voice input) =="
fi
